...
% 5) Pad CDFs to be same x limits for each mouse/folder.
% 6) Calculate the integral of the full 'padded' CDFs.
%% How to Use
% Input
%   -- Give the percent threshold of interest
% Output 
%   -- CDF of cluster sizes and area-under-curve of CDFs for each file.
%% Acknowledgement
% Modified from: Sajid Ali, Sungkyunkwan University, Suwon, South Korea
% Date: |May 31, 2022|
% Contact: sajidali@skku.edu
%% 
function [vq1, auc] = hyperAUCfull(percent)
...
for i = 1:num_frames
    ...
    h1newX{i} = h1.XData(3:2:end-1);
    h1newY{i} = h1.YData(3:2:end-1);
    %figure(1);hold on;plot(h1newX,h1newY); %for checking
    
    %store sizes of each cdf's x-axis
    x_length(i) = h1newX{i}(end);
end  
    
%find longest cdf in x
[maxX,ind] = max(x_length);
%set this to be the length for all cdfs
x = 1:1:maxX;

%auc of cdf
vq1 = cell(num_frames);
for i = 1:num_frames
    %pad shorter cdfs to match length of longest one
    if length(h1newX{i}) < maxX
        filler = maxX - h1newX{i}(end);
        h1newX{i} = [h1newX{i} x(end-filler+1:end)];
        h1newY{i} = [h1newY{i} ones(1,filler)];
    end
    %interpolate CDF
    vq1{i} = interp1(h1newX{i},h1newY{i},x);
    %calculate integral of CDF
    auc(i) = trapz(x,vq1{i});
end
